...
internal class IntGeneratorTest : BaseV1GeneratorTest() {
    @Nested
    inner class Generation {
        ...
        fun `generates both positive and negative integers over multiple runs`() {
            withCounter {
                GenV1.int(-100..100).samples().take(10000).forEach { value ->
                    collect(
                        when {
                            value > 0 -> "positive"
                            value < 0 -> "negative"
    ...
    inner class Shrinking {
        ...
        fun `shrinking zero produces no shrinks`() {
            ...
            val (originalValue, shrinks) = GenV1.int().generateWithShrunkValues(tree)
