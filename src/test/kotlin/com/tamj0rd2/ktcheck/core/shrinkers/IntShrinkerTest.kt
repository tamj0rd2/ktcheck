...
class IntShrinkerTest {
    ...
    // todo: test title is incorrect. it will only work this way if 0 is in range.
    @Test
    ...
    // todo: again, not correct when the shrink target is not 0
    @Test
    fun `when 0 is in range, shrinks are closer to 0 than the original number`() {
        val range = -50..50
        generateSequence { range.random() }.take(100).filter { it != 0 }.forEach { value ->
            val shrinks = IntShrinker.shrink(value, range).toList()
            expectThat(shrinks)
